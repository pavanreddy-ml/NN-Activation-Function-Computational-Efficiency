from config import RESULTS_PATH, COLS
import pandas as pd
import os
...
def create_dataframe(data, dataset_name, activation):
    rows = []
    for i in range(len(data["continuous_activation_time"])):
        new_row = {
            "dataset": dataset_name,
            "activation": activation,
            "run": data["run"][i],
            "continuous_activation_time": data["continuous_activation_time"][i],
            "continuous_train_time": data["continuous_train_time"][i],
            "continuous_performance_metric": data["continuous_loss"][i],
            "piecewise_activation_time": data["piecewise_activation_time"][i],
            "piecewise_train_time": data["piecewise_train_time"][i],
            "piecewise_performance_metric": data["piecewise_loss"][i],
            "piecewise_profile_data": data["piecewise_profile_data"],
            "combined_activation_time": data["combined_activation_time"][i],
            "combined_train_time": data["combined_train_time"][i],
            "combined_performance_metric": data["combined_loss"][i],
            "combinede_profile_data": data["combined_profile_data"]
        }
        rows.append(new_row)
